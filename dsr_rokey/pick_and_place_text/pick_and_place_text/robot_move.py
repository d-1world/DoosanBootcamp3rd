...
tool_dict_path = os.path.join(package_path, "resource", "class_name_tool.json")
# JSON 파일 열기
with open(tool_dict_path, 'r') as f:
    tool_dict = json.load(f)

# JSON 키를 정수로 변환된 새 딕셔너리 만들기 (보호적으로 처리)
tool_dict = {int(k): v for k, v in tool_dict.items()}
